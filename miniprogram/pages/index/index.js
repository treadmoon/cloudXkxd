...
var formatDate = require('../../utils/util.js').formatDate
...
Page({
  ...
  hidePanelInvite: function() {
    ...
    wx.cloud.callFunction({
      ...
      wx.hideToast()
      wx.showToast({
        title: this.data.fontObj.val==''?'你无语了么？':'要文明用语哦！',
        mask: true,
        image: '/imgs/tip.png',
        duration: 2000,
        complete: () => {
          let initval = this.data.fontObj.val == '' ? ' ' : '你人真好！'
          this.setData({
            ['fontObj.val']: initval
          })
          this.setData({
            ['fontObj.txtVal']: initval
          })
        }
      })
    })
  ...
  hidepanel: function() {
    ...
    this.addCustInput(this.data.fontObj.val)
    this.setData({
  ...
  onReady: function() {
    ...
    db.collection('title').where({
      _id: 'XA-MJ1sqTi00tkbX'
    }).get({
      success: res => {
        this.setData({
          ['fontObj.val']: res.data[0].titleVal,
          ['fontObj.txtVal']: res.data[0].titleVal,
          ['fontObj.size']: res.data[0].fontSize
        })
      }
    })
  },
  ...
  addCustInput: function(inp) {
    const db = wx.cloud.database()
    db.collection('custInput').add({
      data: {
        txt: inp,
